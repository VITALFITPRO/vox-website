...
serve(async (req) => {
  ...
  try {
    ...
    // --- RATE LIMITING ---
    const rateStatus = await checkRateLimit(req, supabase, 'validate-download');
    if (!rateStatus.allowed) {
      return new Response(
        JSON.stringify({ error: 'Demasiadas peticiones. Bloqueo Anti-DDoS activo.', code: 'RATE_LIMIT_EXCEEDED' }),
